-- enemy units yield fishes -- original by mewmew modified by kevinma
...
local entities_that_earn_fishes = {
...
local function on_entity_died(event)	
	...
	if not fish_yield[event.entity.name] then return end
	...
	if event.cause then
		if event.cause.valid then
			...
			for _, player in pairs(players_to_reward) do
				player.insert({name = "raw-fish", count = fish_yield[event.entity.name]})
			end
		end
		if entities_that_earn_fishes[event.cause.name] then
			event.entity.surface.spill_item_stack(event.cause.position,{name = "raw-fish", count = fish_yield[event.entity.name]}, true)
			reward_has_been_given = true
		end
	end	
																	
	if reward_has_been_given == false then
		event.entity.surface.spill_item_stack(event.entity.position,{name = "raw-fish", count = fish_yield[event.entity.name]}, true)
	end
end
	
Event.register(defines.events.on_entity_died, on_entity_died)